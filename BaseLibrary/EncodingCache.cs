...
namespace Larpx.ResourceSpider.BaseLibrary
{
    ...
    internal unsafe struct EncodingCache
    {
        ...
        {
            Encoding = encoding;
            if (encoding.CodePage == Utf8.Encoding.CodePage)
                Type = Utf8.Type;
            else if (encoding.CodePage == Ascii.Encoding.CodePage)
                Type = Ascii.Type;
            else if (encoding.CodePage == Unicode.Encoding.CodePage)
                Type = Unicode.Type;
            else if (EncodingCacheOther.Gb2312.Encoding != null && encoding.CodePage == EncodingCacheOther.Gb2312.Encoding.CodePage)
                Type = EncodingCacheOther.Gb2312.Type;
            else if (EncodingCacheOther.Gbk.Encoding != null && encoding.CodePage == EncodingCacheOther.Gbk.Encoding.CodePage)
                Type = EncodingCacheOther.Gbk.Type;
            else if (EncodingCacheOther.Gb18030.Encoding != null && encoding.CodePage == EncodingCacheOther.Gb18030.Encoding.CodePage)
                Type = EncodingCacheOther.Gb18030.Type;
            else if (EncodingCacheOther.Big5.Encoding != null && encoding.CodePage == EncodingCacheOther.Big5.Encoding.CodePage)
                Type = EncodingCacheOther.Big5.Type;
            else Type = 0;
        }
        ...
        internal string GetString(byte[] buffer, int index, int length)
        {
            ...
            if (IsUnicode != 0)
            {
                string text = Extension.StringExtension.FastAllocateString(length >> 1);
            ...
            fixed (byte* bufferFixed = buffer) 
                return Memory.ToString(bufferFixed + index, length);
        }
        ...
        internal byte[] GetBytesNotEmpty(string text)
        {
            ...
            fixed (char* textFixed = text) 
                return Extension.StringExtension.GetBytes(textFixed, length);
        }
        ...
        internal int WriteBytesNotEmpty(string text, byte[] buffer, int index)
        {
            int length = text.Length;
            if ((Type & (1 | 4)) == 0) 
                return Encoding.GetBytes(text, 0, length, buffer, index);
            fixed (char* textFixed = text)
            fixed (byte* bufferFixed = buffer)
            {
                if ((Type & 4) != 0)
                    Memory.CopyNotNull(textFixed, bufferFixed + index, length <<= 1);
                else
                    Extension.StringExtension.WriteBytes(textFixed, length, bufferFixed + index);
            }
            return length;
        }
        ...
        internal int WriteBytesNotEmpty(string text, byte[] buffer)
        {
            int length = text.Length;
            if ((Type & (1 | 4)) == 0)
                return Encoding.GetBytes(text, 0, length, buffer, 0);
            fixed (char* textFixed = text)
            fixed (byte* bufferFixed = buffer)
            {
                if ((Type & 4) != 0) Memory.CopyNotNull(textFixed, bufferFixed, length <<= 1);
                else 
                    Extension.StringExtension.WriteBytes(textFixed, length, bufferFixed);
            }
            return length;
        }
        ...
        internal void WriteBytes(char* start, int count, UnmanagedStream stream)
        {
            ...
            else Extension.StringExtension.WriteBytes(start, size, write);
        ...
        internal void WriteBytes(CharStream charStream, byte[] buffer)
        {
            fixed (byte* bufferFixed = buffer)
            {
                ...
                else Extension.StringExtension.WriteBytes(charStream.Char, buffer.Length, bufferFixed);
            }
        }
        ...
        internal void WriteBytes(CharStream charStream, ref SubArray<byte> buffer)
        {
            fixed (byte* bufferFixed = buffer.Array)
            {
                ...
                else Extension.StringExtension.WriteBytes(charStream.Char, buffer.Length, bufferFixed + buffer.Start);
            }
        }
        ...
        private int writeNumberNotUnicode(char* start, int count)
        {
            if ((Type & 2) != 0)
            {
                Extension.StringExtension.WriteBytes(start, count, (byte*)start);
                return count;
            }
        ...
        internal void Write(byte value, UnmanagedStream stream)
        {
            ...
            int count = Extension.Number.ToString(value, write);
        ...
        internal void Write(uint value, UnmanagedStream stream)
        {
            if ((Type & 4) == 0)
            {
                if ((Type & 2) != 0)
                {
                    ...
                    else stream.ByteSize += Extension.Number.ToBytes(value, stream.GetPrepSizeCurrent(10));
                }
                else
                {
            ...
            else stream.ByteSize += Extension.Number.ToString(value, (char*)stream.GetPrepSizeCurrent(10 * sizeof(char))) * sizeof(char);
        }
        ...
        internal void Write(int value, UnmanagedStream stream)
        {
            if ((Type & 4) == 0)
            {
                if ((Type & 2) != 0)
                {
                    ...
                    else stream.ByteSize += Extension.Number.ToBytes(value, stream.GetPrepSizeCurrent(11));
                }
                else
                {
            ...
            else stream.ByteSize += Extension.Number.ToString(value, (char*)stream.GetPrepSizeCurrent(11 * sizeof(char))) * sizeof(char);
        }
            ...
            if ((Type & 4) == 0)
            {
                if ((Type & 2) != 0)
                {
                    ...
                    else
                    {
                        ...
                        RangeLength index = Extension.Number.ToBytes(value, write);
                        if (index.Start != 0) Memory.SimpleCopyNotNull64(write + index.Start, write, index.Length);
                ...
                else
                {
                    ...
                    int length = Extension.Number.ToString(value, write);
                    int size = Encoding.GetByteCount(write, length);
                    Encoding.GetBytes(write, length, (byte*)write, size);
                    stream.ByteSize += size;
                }
            }
            else stream.ByteSize += Extension.Number.ToString(value, (char*)stream.GetPrepSizeCurrent(20 * sizeof(char))) << 1;
        }
        ...
        internal void Write(long value, UnmanagedStream stream)
        {
            ...
            if ((Type & 4) == 0)
            {
                if ((Type & 2) != 0)
                {
                    Extension.StringExtension.WriteBytes(write, length, (byte*)write);
