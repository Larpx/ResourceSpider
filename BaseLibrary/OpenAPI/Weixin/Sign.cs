...
namespace Larpx.ResourceSpider.BaseLibrary.OpenAPI.Weixin
{
    ...
    internal static class Sign<valueType> where valueType : class
    {
        ...
        unsafe static Sign()
        {
            ...
            {
                setSign = signField == null ? Emit.Property.SetProperty<valueType, string>(signProperty) : Larpx.ResourceSpider.BaseLibrary.Emit.Field.UnsafeSetField<valueType, string>(signField);
                if (count == 0)
                {
