...
namespace Larpx.ResourceSpider.CommonHelper
{
    public class CommonHelper
    {
        ...
        /// <returns></returns>
        public static string DownloadLinkTran(string sLink, string sType)
        {
            try
            {
                ...
                {
                    //加载JS
                    var GetTkkJS = File.ReadAllText("./Scripts/base64.js");

                    switch (sType)
                    {
                        case "xl":
                            //"thunder://" + encode64(strUnicode2Ansi("AA" + url + "ZZ"));
                            sLink = ExecuteScript("encode64(strUnicode2Ansi(\"AA\"" + sLink + "\"ZZ\"))", GetTkkJS);
                            if (!string.IsNullOrEmpty(sLink))
                                return "thunder://" + sLink;
                            else
                                return "";
                        case "qq":
                            //"qqdl://" + encode64(strUnicode2Ansi(url));
                            sLink = ExecuteScript("encode64(strUnicode2Ansi(" + sLink + "))", GetTkkJS);
                            if (!string.IsNullOrEmpty(sLink))
                                return "qqdl://" + sLink;
                            else
                                return "";
                        case "xm":
                            sLink = ExecuteScript("encode64(" + DownloadLinkTran(sLink, "xl") + ")", GetTkkJS);
                            if (!string.IsNullOrEmpty(sLink))
                                return "https://d.miwifi.com/d2r/?url=" + sLink;
                            else
                                return "";
                        default:
                            return sLink;
