...
namespace Larpx.ResourceSpider.Selector.Utils
{
        ...
        public static void FixAllRelativeHref(HtmlDocument document, string url)
        {
            ...
            if (hrefNodes != null)
            {
                foreach (var node in hrefNodes)
                {
                    ...
                    {
                        node.Attributes["href"].Value = UriUtils.CanonicalizeUrl(href, url);
                    }
                }
            }
            ...
            if (srcNodes != null)
            {
                foreach (var node in srcNodes)
                {
                    ...
                    {
                        node.Attributes["src"].Value = UriUtils.CanonicalizeUrl(src, url);
                    }
                }
            }
        }
